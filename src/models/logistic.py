...
from src.models.common import ALL_FEATURES, _add_predictions



def _build_preprocessor(feature_cols):
    """Build the same preprocessing pipeline Carissa defined."""
    num_transformer = Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("scaler", StandardScaler())
    ])
    pre = ColumnTransformer([
        ("num", num_transformer, list(range(len(feature_cols))))
    ])
    return pre


def logistic_model_fn(train_df, test_df, feature_cols=None, label_col="label"):
    """Logistic Regression compatible with the walk-forward pipeline."""
...
def ridge_logistic_model_fn(train_df, test_df, feature_cols=None, label_col="label"):
    """Ridge Logistic Regression compatible with the walk-forward pipeline."""
    feature_cols = feature_cols or ALL_FEATURES
    X_train = train_df[feature_cols].values
    y_train = train_df[label_col].values
    X_test = test_df[feature_cols].values
...
# ── Carissa's standalone script (runs only when executed directly) ────────
